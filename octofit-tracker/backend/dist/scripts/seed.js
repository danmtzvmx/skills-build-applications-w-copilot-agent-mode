...
function makeUnusableSeedPassword() {
...
async function seedDatabase() {
    try {
        ...
        for (const name of teams) {
            const team = await Team.findOneAndUpdate({ name }, { $setOnInsert: { name, members: [] } }, { upsert: true, returnDocument: 'after', runValidators: true });
        ...
        for (const user of users) {
            ...
            const savedUser = await User.findOneAndUpdate({ email: user.email }, {
        ...
        for (const activity of activities) {
            ...
            await Activity.findOneAndUpdate({ user: userId, type: activity.type, date }, {
                $set: {
        ...
        for (const [index, [username, score]] of rankings.entries()) {
            ...
            await Leaderboard.findOneAndUpdate({ user: userId }, { $set: { score, rank: index + 1 }, $setOnInsert: { user: userId } }, { upsert: true, returnDocument: 'after', runValidators: true });
        }
        for (const workout of workouts) {
            await Workout.findOneAndUpdate({ name: workout.name }, { $set: workout }, { upsert: true, returnDocument: 'after', runValidators: true });
        }
